...
#                    by diffing the name lists in brew's API cache. Covers only
#                    the gap between your last two updates, and has no dates.
...
_brew_new_section_local() {
  local title="$1" stem="$2" kind_flag="$3" before after body count desc name
  ...
  body=$(awk 'NR==FNR{a[$0];next} !($0 in a)' "$before" "$after" | sort | while read -r name; do
    [ -n "$name" ] || continue
    if [ "$WANT_DESC" = 1 ]; then
      desc=$(brew desc "$kind_flag" "$name" 2>/dev/null)
      desc=${desc#"$name: "}
      printf '  %-34s %s\n' "$name" "$desc" | sed 's/[[:space:]]*$//'
    else
      printf '  %s\n' "$name"
    fi
  done)
